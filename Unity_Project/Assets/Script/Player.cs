...
public class Player : Unit {
    ...
    void PointingSystem()
    {
		foreach (SixenseHand hand in hands)
		{
			if (hand.m_controller != null && hand.m_controller.GetButton(SixenseButtons.BUMPER))
			{
				RaycastHit hit;
        ...
        foreach (SixenseHand hand in hands)
        {
			if (hand.m_controller != null && destination != Vector3.zero && hand.m_controller.GetButtonUp(SixenseButtons.BUMPER))
            {
				navMeshAgent.Resume ();
